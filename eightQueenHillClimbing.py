...
def find_child(board, n, sideways_move=False):
    ''' Function to find the successor from all the children by comparing the heuristic values of moving the queens
    row-wise '''
    child = []
    ...
    for row in range(n):
        for col in range(n):
            ...
            if (sideways_move):
                # if sideways moves are allowed, and the generated child heuristic cost is less than or equal to the
                # current lowest heuristic cost, save generated child and update current lowest heuristic cost
                if (temp_board != board):
                ...
                # if sideways moves are not allowed, and the generated child heuristic cost is less than the current
                # lowest heuristic cost, save generated child and update current lowest heuristic cost
...
for i in range(iterations):
    ...
    print("Gecen Sure: {:.5f}s".format(time.time() - start_time))

print('Basari orani: ' + str(success_rate_steepest_hill_climbing / iterations))
print('Basarisiz olma orani: ' + str(1 - (success_rate_steepest_hill_climbing / iterations)))
print('Basarili olma durumunda ortalama adim sayisi: ' + str(
    step_count_rate_steepest_hill_climbing_success / success_rate_steepest_hill_climbing))
print('Basarisiz olma durumunda ortalama adim sayisi: ' + str(
